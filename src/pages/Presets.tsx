...
import { PresetArea, PresetItem, Preset } from "@/types/checklist";
...
const Presets = () => {
  ...
  // Detectar se é modo create ou edit pela URL
  useEffect(() => {
    if (location.pathname === '/presets/new') {
      setFormData({
        nome: "",
        descricao: "",
        areas: []
      ...
      setActiveTab('dados');
      setViewMode('create');
    } else if (presetId && presets.length > 0) {
      const preset = presets.find(p => p.id === presetId);
      if (preset) {
        handleEdit(preset);
      }
    }
  }, [location.pathname, presetId, presets]);
  ...
  const fetchPresets = async () => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        // Normalizar campos de data (suportar ambos os formatos)
        const createdAt = data.criadoEm?.toDate() || data.createdAt?.toDate() || new Date();
        const updatedAt = data.atualizadoEm?.toDate() || data.updatedAt?.toDate() || new Date();
  ...
  const handleCreateNew = () => {
    ...
    setActiveTab('dados');
    setViewMode('create');
    // Navegar mantendo no Dashboard
    navigate('/presets/new', { state: { activeTab: 'presets' } });
  };
  ...
  const handleEdit = (preset: Preset) => {
    ...
    setViewMode('edit');
    // Navegar mantendo no Dashboard
    navigate(`/presets/${preset.id}`, { state: { activeTab: 'presets' } });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const dataToSave = {
        ...
        updatedAt: now,
        // Manter formato em português também para compatibilidade
        atualizadoEm: now
      };
      ...
      await fetchPresets();
      setViewMode('list');
      navigate('/dashboard', { state: { activeTab: 'presets' } });
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          className="bg-cerrado-green1 hover:bg-cerrado-green2 text-white"
        >
                ...
                <tr key={preset.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center">
                      <FolderOpen className="h-5 w-5 text-cerrado-green1 mr-3 flex-shrink-0" />
    ...
    <div className="space-y-6 min-h-[calc(100vh-200px)]">
      ...
      <div className="flex items-center gap-4 pb-4 border-b">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-cerrado-green1">
      ...
      <div className="flex gap-8 border-b">
        <button 
          ...
          className={`pb-3 px-1 text-sm font-medium border-b-2 transition-colors ${
            activeTab === 'dados' 
              ? 'border-cerrado-green1 text-cerrado-green1' 
          ...
          <div className="space-y-4 flex-1 overflow-y-auto max-h-[calc(100vh-380px)] pr-2">
            {formData.areas.map((area, areaIndex) => (
              <div key={area.id} className="bg-white border border-gray-200 rounded-lg shadow-sm relative">
                {/* Área Header - canto verde */}
                <div className="absolute top-0 left-0 bg-cerrado-green1 rounded-tl-lg px-4 py-2 flex items-center gap-2 max-w-[90%] md:max-w-[70%] lg:max-w-[500px]">
                  <button
                      ...
                      {area.items.length === 0 ? (
                        ...
                        area.items.map((item, itemIndex) => (
                          <div key={item.id} className="bg-gray-50/80 border border-gray-200 rounded-lg shadow-sm relative min-h-[72px]">
                            ...
                            <div className="absolute top-0 left-0 right-0 bg-cerrado-green1 rounded-lg px-3 py-2 flex items-start gap-2 min-h-[40px] max-h-[72px] overflow-hidden">
                              <button
