...
exports.resetClientThread = exports.migrateClientes = exports.testWhatsappFlow = exports.whatsappWebhook = void 0;
...
exports.whatsappWebhook = functions.https.onRequest(async (req, res) => {
    ...
    corsHandler(req, res, async () => {
        try {
            ...
            if (!zapiService_1.zapiService.isValidReceivedMessage(webhookPayload)) {
            ...
            const messageInfo = zapiService_1.zapiService.extractMessageInfo(webhookPayload);
            ...
            const cliente = await clienteService_1.clienteService.findByPhone(messageInfo.phone);
            if (!cliente) {
                functions.logger.info('❌ Cliente não encontrado no sistema', {
                    phone: messageInfo.phone
                });
                // Enviar mensagem de acesso negado
                await zapiService_1.zapiService.sendText({
                    phone: messageInfo.phone,
            ...
            const isActive = await clienteService_1.clienteService.isClienteActive(cliente);
            if (!isActive) {
            ...
            const clientContext = clienteService_1.clienteService.createClientContext(cliente);
            // Processar mensagem com OpenAI (passando clienteId para gerenciar thread)
            const assistantResponse = await openaiService_1.openaiService.processMessage(messageInfo.message, clientContext, cliente.id);
            ...
            await clienteService_1.clienteService.logInteraction(cliente.id, {
                message: messageInfo.message,
            ...
            await clienteService_1.clienteService.updateLastAccess(cliente.id);
 ...
 */
exports.testWhatsappFlow = functions.https.onRequest(async (req, res) => {
    corsHandler(req, res, async () => {
        try {
 ...
 */
async function processWhatsappMessage(payload) {
    const messageInfo = zapiService_1.zapiService.extractMessageInfo(payload);
 ...
 */
exports.migrateClientes = functions.https.onRequest(async (req, res) => {
    corsHandler(req, res, async () => {
        try {
 ...
 */
exports.resetClientThread = functions.https.onRequest(async (req, res) => {
    corsHandler(req, res, async () => {
        try {
            if (req.method !== 'POST') {
            ...
            if (clienteId) {
                ...
                if (clienteDoc.exists) {
                    ...
                    cliente = Object.assign({ id: clienteDoc.id }, clienteData);
                }
            }
            else if (phone) {
                // Buscar por telefone
                cliente = await clienteService_1.clienteService.findByPhone(phone);
            }
            if (!cliente) {
            ...
            // Resetar thread
            const newThreadId = await openaiService_1.openaiService.resetClientThread(cliente.id);
